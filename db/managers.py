...
class ManagersScraper:
    ...
    def update_user(self, user: User):
        self.cur.execute(f"UPDATE {self.table_name} SET history = %s WHERE id = %s", (json.dumps(user.history), user.id,))
        self.conn.commit()
    ...
    def get_statistic(self):
        ...
        for record in records:
            users.append(User(id=record[0], name=record[1], history=json.loads(record[2])))
        return users
