...
class MarketPlaceScraper:
    ...
    def get_source_item(self):
        ...
        if record:
            ...
            return SourceItem(id=record[0], link=record[1], photo=record[2],
                              name=record[3], price=record[4], creation_date=record[5])
        return None
    ...
    def save_to_error_mk(self, table_name, source_item: SourceItem, now):
        self.cur.execute(
            ...
            (source_item.id, source_item.link, source_item.photo,
             source_item.name, source_item.price, source_item.creation_date, now,
             ))
        self.conn.commit()

    def save_to_neural(self, table_name, mk_item: MarketPlaceItem):
        data = mk_item.source_item
        self.cur.execute(f"""INSERT INTO {table_name} (id, link, photo, name, 
